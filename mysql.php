...
Class MySQL
{
	...
	function init ()
	{
		...
		if(!$conn) {
			$this->error("mysql დაკავშირების მცდელობა წარუმატებელი აღმოჩნდა");
		}
		if(!mysql_select_db($dbase,$conn)) {
			$this->error("db შერჩევა ვერ მოხერხდა");
		}
		$this->CONN = $conn;
		return true;
	}
...
//	*****************************************************************
	...
	function select ($sql="", $column="")
	{
		...
		{
			echo "<H2>არასწორი შერჩევის ფუნქცია! $sql</H2>\n";
			return false;
		}
	...
	function insert ($sql="")
	{
		...
		{
			echo "<H2>არასწორი ჩასმის ფუნქცია! $sql</H2>\n";
			return false;
		}
		if(empty($this->CONN))
		{
			echo "<H2>არ არის კავშირი!</H2>\n";
			return false;
		}
		$conn = $this->CONN;
		$results = mysql_query($sql,$conn);
	...
	function sql_query ($sql="")
	{
		...
		if(!$results)
		{
			echo "<H2>კითხვა ცუდად წარიმართა!</H2>\n";
			echo mysql_errno().":  ".mysql_error()."<P>";
			return false;
		}
		return $results;
...
//	The power of recursive queries
	...
	{
		if(empty($LinkID)) {
			$err_msg = "არ არის მითითებული LinkID.";
			return false;
		}
	...
	function suggest ($postData="",&$err_msg)
	{
		...
		if(!$this->isValidCatID($CatID)) {
			$err_msg = "არასწორი კატეგორია.";
			return false;
		}
		if(empty($Url)) {
			$err_msg = "URL მითითებული არ არის.";
			return false;
		}
		if(empty($Description)) {
			$err_msg = "აღწერა არ არის მითითებული.";
			return false;
		}
		if(empty($LinkName)) {
			$err_msg = "ლინკის სახელი არ არის მითითებული.";
			return false;
		}
		if(empty($SubmitName)) {
			$err_msg = "სახელი არ არის მითითებული.";
			return false;
		}
		if(empty($SubmitEmail)) {
			if ($REQUIRE_SUBMIT_EMAIL) {
				$err_msg = "ელექტრონული ფოსტის მისამართი არ არის მითითებული.";
				return false;
			} else {
				$SubmitEmail = "ანონიმური";
			}
		}
	...
	function update ($postData="",&$err_msg)
	{
		...
		if( (empty($postData)) or (!is_array($postData)) ) {
			$err_msg = "არ არის წარმოდგენილი მონაცემები ან არ არის მონაცემთა მასივი";
			return false;
		}
	...
	function add_cat ($CatParent,$NewCatName,&$err_msg)
	{
		...
		if(empty($NewCatName)) {
			$err_msg = "კატეგორიის ახალი სახელი არ არის მითითებული.";
			return false;
		}
