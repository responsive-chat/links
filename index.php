...
function start_browse($CatID="")
{
	...
	if ("$ADMIN_MODE"=="true"){
		print "\n\n";
		print "<CENTER><b>წარდგინებები</b></CENTER>\n";
		show_submissions_list($CatID);
		$CatID = $OurCatID;	// restore CatID

		// Show form to add a subcategory
		print "\n\n";
		print "<CENTER><b>ახალი კატეგორია</b></CENTER>\n";
		print "<p><center>
...
function show_edit_link($LinkID="",$title="",$msg="")
{
	...
	print "<p><b>&#9783; კატეგორია: <B>$LinkCatName</B></b>
	<form action=\"".$_SERVER['PHP_SELF']."?update=1\" method=\"POST\">
	<input type=\"hidden\" name=\"LinkID\" value=\"$LinkID\">
	<table align=\"center\" border=\"0\">
	<tr><td><input name=\"Url\" placeholder=\"ვებ-საიტის ბმული\" size=\"80\" VALUE=\"$Url\"></td></tr>
	...
	print "<tr><td><br>&nbsp;&nbsp; $SDate &nbsp;&nbsp;<br></td></tr>
	<tr><td><br><input type=\"submit\" name=\"update\" value=\"ბმულის განახლება\">
...
function show_add_link($add = "NULL", $CatName = "Unknown")
{
	...
	print_header($add);
	print "
	<p><b>&#9783; კატეგორია: <B>$CatName</B></b>
	<form action=\"".$_SERVER['PHP_SELF']."?suggest=1\" method=\"POST\">
	<input type=\"hidden\" name=\"CatID\" value=\"$add\">
	<table align=\"center\" border=\"0\">
	<tr><td><input name=\"Url\" placeholder=\"ვებ-საიტის ბმული\" size=\"80\" VALUE=\"http://\"></td></tr>
	...
	if( (!$hits) or (empty($hits)) )
	{
		...
		while ( list ($key,$hit) = each ($hits))
		{
			if(!empty($hit))
			{
				...
				print "<UL>\n";
				print "<LI><A HREF=\"$LinkURL\" TARGET=\"_NEW\">$LinkName</A>\n";
				print "<DD>$LinkDesc\n";
